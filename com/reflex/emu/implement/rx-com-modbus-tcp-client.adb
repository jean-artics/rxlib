...
package body Rx.Com.Modbus.Tcp.Client is

   package LibModbus renames Rx.Com.Protocols.LibModbus;

   protected body Client_Status is
      ...
      procedure Set_Connected (Value : in Boolean) is
      begin
         Status.Connected := Value;
      end Set_Connected;

      function Get_Connected return Boolean is
      begin
      ...
      function Get_Status return Client_Status_Type is
      begin
         return Status;
      end Get_Status;
   ...
   procedure Run
	...
	"Rx.Com.Modbus.Tcp_Client.Periodic_Task.Run "
	...
	Milliseconds (Configuration.Task_Period_MS);
      ...
      subtype Buffer_Bool_T is Bool_Array (0 .. MODBUS_MAX_READ_BITS - 1);
      ...
      procedure Close is
         My_Ident : constant String := "Rx.Com.Modbus.Tcp.Run.Close";
      begin
         if Connect_Ok then
            LibModbus.Close (Context => MyContext);
            Connect_Ok := False;
            Client_Itf.Status.Set_Connected (False);
   ...
   begin
      ...
      loop
         ...
         Client_Itf.Status_Watchdog.Watchdog
           (Watching         => Client_Itf.Control.Start_Watching,
            Control_Watchdog => Client_Itf.Control_Watchdog.Value,
            Error            => Watchdog_TON_Q);

         if Watchdog_TON_Q and not Watchdog_Error then
            Rx.Sys.Log.Logger.Put
	      (Who  => My_Ident, What => "Watchdog Time Out elapsed!");
            Watchdog_Error := True;
         end if;
         ...
         if Configuration.Enabled then
            ...
            if not Context_Ok then
               ...
               LibModbus.Set_Debug
		 (Context => MyContext, On => Configuration.Debug_On);
            ...
            if not Connect_Ok then
               ...
               Connect_Ok := True;
               Client_Itf.Status.Set_Connected (True);
            else
               ...
               if Faults_TON_Q then
                  ...
                  loop
                     if Fault = Client_Itf.Status.Get_Command_Status (Index) 
		     then
                        Client_Itf.Status.Set_Command_Status (Index, Unknown);
                        Client_Itf.Status.Set_Command_Retries
               ...
               for Index in Configuration.Commands'Range loop
                  ...
                  Command_Status := Client_Itf.Status.Get_Command_Status(Index);
                  ...
                  if Command_Status = Unknown or Command_Status = Fine then
                     ...
                     if Command_Scheduling (Index) > 0 then
                        ...
                        Client_Itf.Status.Set_Command_Status (Index, Fine);
         ...
         exit when Client_Itf.Control.Quit;
   ...
   exception
      ...
      when Error : LibModbus.Context_Error | LibModbus.Connect_Error =>
         ...
         for Index in Configuration.Commands'Range
         loop
            if not (Disabled = Client_Itf.Status.Get_Command_Status (Index)) 
	    then
               Client_Itf.Status.Set_Command_Status (Index, Unknown);
            end if;
         end loop;
      ...
      when Error : others =>
	   ...
	   (Who  => My_Ident,
	    ...
	    then
         ...
         Command_Retries :=
           Client_Itf.Status.Get_Command_Retries (Command_Index);

         if Command_Retries > 0 then
            Client_Itf.Status.Set_Command_Retries
              (Command_Index, Command_Retries - 1);
         else
            Client_Itf.Status.Set_Command_Status (Command_Index, Fault);
         end if;
   ...
   task body Client_Task is
      My_Ident : constant String := "Rx.Com.Modbus.Tcp.Cient.Client_Task "
        & To_String (Configuration.Server_IP_Address) & ":"
        & Configuration.Server_TCP_Port'Img;
   begin
      ...
      loop
         if not Configuration.Commands (Index).Enabled then
            Client_Itf.Status.Set_Command_Status (Index, Disabled);
         end if;
         Client_Itf.Status.Set_Command_Retries (Index, Configuration.Retries);
      end loop;

      loop
         Run (Configuration, Client_Itf, Bool_DPM_Access, Word_DPM_Access);
      ...
      Client_Itf.Status.Set_Terminated (Client_Itf.Control.Quit);
   end Client_Task;
